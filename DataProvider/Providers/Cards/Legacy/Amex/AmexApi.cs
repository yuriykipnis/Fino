...
namespace DataProvider.Providers.Cards.Amex
{
    public class AmexApi : IAmexApi
    {
        ...
        internal static ExpensesInfo RetriveExpensesInfo(string response)
        {
            ...
            var abroadTransactions = data.CardsTransactionsListBean.Index.CurrentCardTransactions.Last().TxnAbroad;
            if (transactions != null && abroadTransactions != null)
            {
                transactions = transactions.Concat(abroadTransactions).ToList();
            }
            ...
            {
                Transactions = transactions ?? abroadTransactions ?? new List<Dto.CardTransaction>(),
